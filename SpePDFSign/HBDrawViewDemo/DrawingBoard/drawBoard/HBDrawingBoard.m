...
@property(assign)BOOL  m_isDrawing;
...
- (instancetype)initWithFrame:(CGRect)frame withPDFUrl:(NSURL *)pdfUrl
{
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserverForName:ImageBoardNotification object:nil queue:nil usingBlock:^(NSNotification * _Nonnull note) {
            ...
            [weakSelf.backImage setImage:img];
        }];
...
//        接受画笔修改的通知
        ...
        [[NSNotificationCenter defaultCenter] addObserverForName:@"SpeZoomImageView" object:nil queue:nil usingBlock:^(NSNotification * _Nonnull note) {
            
            NSString *flag = note.object;
            self.m_isDrawing = [flag integerValue] == 1;
...
- (SpeZoomImageView *)backImage
{
    if (!_backImage) {
        _backImage = [[SpeZoomImageView alloc] initWithFrame:self.bounds];
    }
    return _backImage;
}
...
//        NSString* const frameworkBundleID  = @"Points.SpePDFUtil";
...
//                    UIImageWriteToSavedPhotosAlbum([self screenshot:self], nil, nil, nil);
                    ...
                    case setTypeQuit:
                {
                    ...
                    if(result){
                        self.documentController = [UIDocumentInteractionController interactionControllerWithURL:[NSURL fileURLWithPath:pdfPath]];
//                        documentController.delegate = self;
                        self.documentController.UTI = @"com.adobe.pdf";
                        [self.documentController presentOpenInMenuFromRect:CGRectZero inView:self animated:YES];
                    }
                    else
                    {
...
- (ZXCustomWindow *)drawWindow
{
    if (!_drawWindow) {
        ...
        _drawWindow.m_delegate = self.delegate;
    }
    return _drawWindow;
}
- (void)dealloc
